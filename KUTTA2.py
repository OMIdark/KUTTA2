...
def moch_yayan():
    ...
    if pepek == '':
        ...
        time.sleep(1)
        os.system('rm -rf .memek.txt')
        jalan('\n %s[%s%s]%s berOMI menghapus token' % (N, H, N, H))
        exit()
    else:
            ...
            while True:
                ...
                def __yan__(ysc = (None, None)):
                    ...
                    if cin == '':
                        ...
                        print '\n [%s+%s] OMI OK file -> results/OK-%s-%s-%s.txt' % (O, N, ha, op, ta)
                        print ' [%s+%s] OMI CP file-> results/CP-%s-%s-%s.txt' % (O, N, ha, op, ta)
